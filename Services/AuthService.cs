...
namespace Polyclinic.Services
{
    public class AuthService
    {
        ...
        public async Task<String> Register(Person person)
        {
            ...
            using var response = await client.PostAsync("https://localhost:7122/register", content);
        ...
        {
            JsonContent content = JsonContent.Create(person);
            using var response = await client.PostAsync("https://localhost:7122/login", content);
            string responseText = await response.Content.ReadAsStringAsync();
            if(responseText !="")
            {
